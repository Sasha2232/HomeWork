...
class BbCreateView(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    success_message = 'Объявление о продажи "%(title)s" создано'
...
def edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if bbf.is_valid():
            if bbf.has_changed():
                bbf.save()
                messages.add_message(request, messages.SUCCESS, 'Объявление исправлено!', extra_tags='alert alert-success')
                messages.success(request, 'Объявлние исправлено - 2!', extra_tags='alert alert-success')
